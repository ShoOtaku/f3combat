...
class AstLogic(Strategy):
    ...
    def global_cool_down_ability(self, data: 'LogicData') -> UseAbility | UseItem | UseCommon | None:
        ...
        '''
        for member in alive_member:
            if data.me.level >= 42 and can_swing and member.current_hp < 30000:
        ...
        if can_swing:
            ...
            if member_in_distance_15_hpp_lt_85:
                if data.me.level >= 42: #and is_white or is_black
                    status_id = s('阳星相位') #if is_white else 837
        ...
        for member in alive_member:
            ...
            if can_use_single_status and member.job.is_tank and member.effects.has(s('吉星相位'))  < 3:
                return UseAbility(a('吉星相位'), member.id)           
            if not can_use_3614 and hp_percent(member) < 0.3:
                if can_swing:
                    if data.me.level >= 26:
                        return UseAbility(3610, member.id)#福星
                    else:
                        return UseAbility(3594, member.id)
                if not can_swing:
                    return UseAbility(a('吉星相位'), member.id)#吉星相位
    ...
    def non_global_cool_down_ability(self, data: 'LogicData') -> UseAbility | UseItem | UseCommon | None:
        ...
        if alive_member:
            #天星冲日
            if not data[16553] and sum(hp_percent(member) < 0.85 and
                                        data.actor_distance_effective(member) <= 15
            ...
            if data[3614] <= (30 if data.me.level >= 78 else 0):
                ...
                if hp_percent(member_lowest_hpp) < 0.3:
            ...
            #光速
            if not data[a('光速')]:
                    return UseAbility(a('光速'))
            #擢升
            if not data[25873]:
                member_lowest_hpp = min(alive_member, key=hp_percent)
                if hp_percent(member_lowest_hpp) < 0.5:
                    return UseAbility(25873, member_lowest_hpp.id)
            #占卜
            if not data[16552]:
            ...
            if not data[7439]:
                #if 1224 not in data.effects and 1248 not in data.effects:
                return UseAbility(7439, data.me.id)
                #else:
                    #return UseAbility(8324)
